window.addEventListener("DOMContentLoaded", () => {
  ...
  // ※ server.js の mapLimit と同じロジック（クライアント側コピー）
  ...
  // ===== チャンピオン統計ホスト（DUOの下）=====
  function ensureSideChampHost() {
    if (!side) return null;

    let champ = side.querySelector("#sideChampHost");
    if (!champ) {
      champ = document.createElement("div");
      champ.id = "sideChampHost";

      const duoHost = ensureSideDuoHost();
      if (duoHost && duoHost.nextSibling) side.insertBefore(champ, duoHost.nextSibling);
      else if (duoHost) side.appendChild(champ);
      else side.appendChild(champ);
    }
    return champ;
  }
  ...
  function renderChampPanel(ddragon, champMap, jaChampNames = {}) {
    ...
    const sorted = Array.from(champMap.entries())
      .map(([name, s]) => {
        ...
        return { name, games: s.games, wins: s.wins, losses: s.games - s.wins, wr };
      })
      .filter((x) => x.games >= 1)
    ...
    const rows = sorted.map((x) => {
      ...
      const barColor = x.wr >= 60 ? "rgba(90,160,255,.85)" : x.wr >= 50 ? "rgba(90,200,120,.75)" : "rgba(210,80,80,.75)";
      return `
  ...
  function tierIconSrc(entry) {
    ...
    const rankMap = {
      unranked: "/rank-icons/256/00_unranked_256.png",
      iron: "/rank-icons/256/01_iron_256.png",
      bronze: "/rank-icons/256/02_bronze_256.png",
      silver: "/rank-icons/256/03_silver_256.png",
      gold: "/rank-icons/256/04_gold_256.png",
      platinum: "/rank-icons/256/05_platinum_256.png",
      emerald: "/rank-icons/256/06_emerald_256.png",
      diamond: "/rank-icons/256/07_diamond_256.png",
      master: "/rank-icons/256/08_master_256.png",
      grandmaster: "/rank-icons/256/09_grandmaster_256.png",
      challenger: "/rank-icons/256/10_challenger_256.png",
    };
  ...
  function renderSidePanel(ranked) {
    ...
    const wrBarHtml = (entry) => {
      if (!entry?.tier) return '';
      const w = Number(entry.wins || 0);
      const l = Number(entry.losses || 0);
      const wr = w + l > 0 ? Math.round((w / (w + l)) * 100) : 0;
      const color = wr >= 60 ? "rgba(90,160,255,.85)" : wr >= 50 ? "rgba(90,200,120,.75)" : "rgba(210,80,80,.75)";
      return `
        <div class="wr-bar-wrap">
          <div class="wr-bar" style="width:${wr}%;background:${color};"></div>
        </div>
        <div class="wr-bar-label">${escapeHtml(sideSub(entry))}</div>
      `;
    };
    ...
    host.innerHTML = `
      <div class="side-card">
        ...
        .map((x) => {
          ...
          const duoWrColor = wr >= 60 ? "rgba(90,160,255,.85)" : wr >= 50 ? "rgba(90,200,120,.75)" : "rgba(210,80,80,.75)";
          return `
  <a class="rank-row duo-row"
    ...
    <div class="${nameCls}" title="${escapeHtml(x.name)}">
      ...
      <div class="wr-bar-wrap" style="margin-top:3px;">
        <div class="wr-bar" style="width:${wr}%;background:${duoWrColor};"></div>
      </div>
    </div>
...
`;
  ...
  async function runSearch(riotIdRaw) {
    ...
    try {
      ...
      // ページタイトルと OGP を動的に更新（ブックマーク・SNS シェア対応）
      document.title = `${sum.name}#${sum.tag} - Instant.gg`;
      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) ogTitle.setAttribute("content", `${sum.name}#${sum.tag} - Instant.gg`);
      const ogDesc = document.querySelector('meta[property="og:description"]');
      if (ogDesc) ogDesc.setAttribute("content", `${sum.name}#${sum.tag} のランク・試合履歴を確認`);
      ...
      // ⑤ チャンピオン統計の集計（追加 API なし・履歴データ再利用）
      ...
      const appendNext = async () => {
        ...
        try {
          ...
          for (let i = 0; i < batch.length; i++) {
            ...
            // ⑤ チャンピオン統計を集計（リメイクは除外）
            if (!m.remake && m.championName) {
              const champId = champIdForDdragon(champMaps, m.championName);
              const cur = champStats.get(champId) || { games: 0, wins: 0 };
              cur.games++;
              if (m.win) cur.wins++;
              champStats.set(champId, cur);
            }
          }
