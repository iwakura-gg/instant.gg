...
async function getMatchV5(matchId) {
  ...
  const r = await riotFetch(regionUrl(`/lol/match/v5/matches/${encodeURIComponent(matchId)}`));
...
app.get("/api/summary/:name/:tag", async (req, res) => {
  ...
  const summoner = await riotFetch(platformUrl(`/lol/summoner/v4/summoners/by-puuid/${encodeURIComponent(puuid)}`));
...
app.get("/api/ranked/:summonerId", async (req, res) => {
  ...
  const r = await riotFetch(platformUrl(`/lol/league/v4/entries/by-summoner/${encodeURIComponent(summonerId)}`));
...
app.get("/api/ranked-by-puuid/:puuid", async (req, res) => {
  ...
  const r = await riotFetch(platformUrl(`/lol/league/v4/entries/by-puuid/${encodeURIComponent(puuid)}`));
  if (!r.ok) return res.status(r.status).json({ error: "league api error", detail: r.data });
...
app.get("/api/live/:id", async (req, res) => {
  ...
  // ① PUUID向け（推奨）
  const byPuuid = await riotFetch(platformUrl(`/lol/spectator/v5/active-games/by-puuid/${encodeURIComponent(id)}`));
  ...
  // ② フォールバック
  const bySummoner = await riotFetch(platformUrl(`/lol/spectator/v5/active-games/by-summoner/${encodeURIComponent(id)}`));
...
app.get("/api/matches/:puuid", async (req, res) => {
  ...
  const result = await riotFetch(regionUrl(`/lol/match/v5/matches/by-puuid/${encodeURIComponent(puuid)}/ids?count=20`));
